...
import com.billcom.apc.resiliation.workItem.SmsNotificationWorkItemHandler;
...
public class Client {
    ...
    private String from= resiliationService.getConfig().getPropValues("SMS_FROM");

    private String priority=resiliationService.getConfig().getPropValues("SMS_priority");

    private String validity=resiliationService.getConfig().getPropValues("SMS_validity");

    private String text=resiliationService.getConfig().getPropValues("SMS_text");
    
    private String canal=resiliationService.getConfig().getPropValues("SMS_canal");
    
    private String bid=resiliationService.getConfig().getPropValues("SMS_bid");
    
    private int type=Integer.parseInt(resiliationService.getConfig().getPropValues("SMS_type")); 
...
public static boolean smsNotification(MessageNotif messageNotif,long order_id) {
	Logger logger = Logger.getLogger(Client.class);
		...
		try {
			...
			ResiliationConfigHandler config = new  ResiliationServiceImp().getConfig();
			Client c = new Client();   
